...
class DVFStrain(Env):
    def __init__(self, initSleep, experiment, qos_type: str):
        ...
        self.observation_space = spaces.Box(low=0, high=100, shape=(28, ), dtype=np.float64)
        ...
        states = np.concatenate([[reward, fps, power, ppw], temps[:4], freqs/30000, [little, mid, big, gpu], [little_u, mid_u, big_u], [gpu_util[0]*100], c_states, [11, 14, 17, 12]])
    ...
    def step(self, action):
        ...
        states = np.concatenate([[reward, fps, power, ppw], temps[:4], freqs/30000, [little, mid, big, gpu], [little_u, mid_u, big_u], [gpu_util[0]*100], c_states, action])
        self.state = states
        obs = states
