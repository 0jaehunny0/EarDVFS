...
def action_to_freq(action, c_states):
    ...
    li = int(min(action[0], 100))
    mi = int(min(action[1], 100))
    bi = int(min(action[2], 100))
    gi = int(min(action[3], 100))
...
class DVFStrain(Env):
    def __init__(self, initSleep, experiment, qos_type: str):
        ...
        states = np.concatenate([[reward, fps, power, ppw], temps[:4], freqs/30000, [little, mid, big, gpu], [little_u, mid_u, big_u], [gpu_util[0]*100], [11, 14, 17, 12]])
    ...
    def step(self, action):
        ...
        states = np.concatenate([[reward, fps, power, ppw], temps[:4], freqs/30000, [little, mid, big, gpu], [little_u, mid_u, big_u], [gpu_util[0]*100], action])
        self.state = states
        obs = states
