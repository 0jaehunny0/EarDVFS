...
all_temps = [[] for _ in range(num_sensors)]
...
turn_on_screen()
run_name = f"{int(time.time())}__tempChange__{args.tempSet}__exp{args.experiment}__temp{args.temperature}"
...
for i in range(total_timesteps):
    ...
    for k in range(num_sensors):
        all_temps[k].append(alltempLi[k])
    ...
    if i % 10 == 0 and i != 0:
        ...
        for ik in range(num_sensors):
            writer.add_scalar("tempAll/" + sensor_list[ik], np.array(all_temps[ik])[-10:].mean(), i)
