...
mid_available_frequencies = [400000, 553000, 696000, 799000, 910000, 1024000, 1197000, 1328000, 1491000, 1663000, 1836000, 1999000, 2130000, 2253000, 2348000]
...
gpu_available_frequencies = [202000, 251000, 302000, 351000, 400000, 471000, 510000, 572000, 701000, 762000, 848000]
...
def get_states2(window, qos_type: str, qos_time_prev: float, byte_prev: Optional[int], packet_prev: Optional[int]):
    ...
    msg += '&& cat /proc/stat'
    msg += '&& cat /sys/devices/platform/28000000.mali/utilization'
    ...
    t1 = int(result[10][2:])
    big_e = int(result[15].split()[1]) #+5
    mid_e = int(result[16].split()[1])
    little_e = int(result[17].split()[1])

    t2 = int(result[23][2:]) #+4
    gpu_e = int(result[28].split()[1])

    li = []
    for i in result[37:45]: #+8
        temp = np.array(i.split()[1:], dtype=np.int32)
        li.append([temp[0:7].sum(), temp[3]])

    gpu_util = [int(result[52])/100]

    little = int(result[53])
    mid = int(result[54])
    big = int(result[55])
    gpu = int(result[56])
