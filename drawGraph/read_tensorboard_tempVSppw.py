...
for subdir, dirs, files in os.walk("selected/tempVSppw2"):
    a = subdir.split("/")
...
ax = df.plot(kind = "bar", secondary_y = "GPU", figsize=(6, 3.5), rot=0)
...
ax.right_ax.set_ylim(0, 75)
ax.set_ylim(ax.right_ax.get_ylim())
...
tempDict3 = {}
# for subdir, dirs, files in os.walk("selected/tempVSppw"):
for subdir, dirs, files in os.walk("selected/tempVSppw3"):
    a = subdir.split("/")
...
labels_right = ["GPU"]
ax.legend(handles + handles_right, labels + labels_right, loc="upper center", bbox_to_anchor=(0.5, 1.25), ncol=2, frameon=False, borderpad=0.1, columnspacing=0.5)
ax.set_xticklabels(["20°C", "30°C", "40°C"])
ax.set_xlabel("Non-processor components temperatures")
ax.set_ylabel("CPU temperature (°C)")
ax.get_ylim()
right_ax_label = ax.right_ax.set_ylabel("GPU temperature (°C)")
ax.right_ax.set_ylim(0, 75)
...
# ax = df.plot(kind = "bar", secondary_y = "Power", figsize=(8.5, 4), rot=0)
ax = df.plot(kind = "bar", secondary_y = "GPU", figsize=(8, 3.3), rot=0)
